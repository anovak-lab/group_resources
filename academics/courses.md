...
### Parallel Programming (CS 484, replaced CSE 402/CS 420/ECE 492)

This class introduces techniques for *performant* massively parallel programming. Topics of use to members of the M&Ms group include: historical & modern CPU architectures (with a focus on cache and memory design), shared memory parallelism (primarily OpenMP, though pthreads & C++ threads/atomics are mentioned), MPI parallelism, parallel debugging+profiling, performance modelling for shared/distributed memory parallel algorithms, and a brief section on accelerators (GPUs). The class is fully virtual and has a fairly standard workload: weekly homeworks, biweekly assignments, three computer projects, a four credit hour project (required for graduate students), a midterm, and a final exam. The lectures are pre-recorded videos which provide a nice mix of theory, API use, and programming examples. Be prepared to spend a substantial amount of time on the last computer project and the four credit hour project. Everyone in the group should take this course as it covers topics that are extremely relevant to developing software to run at peta/exascale.
